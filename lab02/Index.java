...
public class Index {
    ...
    public static class WordCountMap extends Mapper<Text, Text, Text, LongWritable> {
		...
		Text output_key = new Text(word.toString() + "," +  key.toString());
                context.write(output_key, new LongWritable(count));
		count++;
            }
        }
    }
     ...
     * pair (word, sum).
     */
    public static class SumReduce extends Reducer<Text, LongWritable, Text, Text> {
	    ...
	    String[] keys = key.toString().split(",");
            String output = keys[1] + ":";
            for (LongWritable value : values) {
                output = output + value + ",";
            }
            context.write(new Text(keys[0]), new Text(output));
        }
    }
